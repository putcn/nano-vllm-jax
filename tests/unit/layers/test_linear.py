...
Tolerances: atol=1e-4 float32 (matmul accumulation).
"""
import pytest
import numpy as np
import jax
...
def test_column_parallel_shape(tp_size, tp_rank):
    ...
    out = layer(jnp.ones((4, in_f)))
    assert out.shape == (4, out_f // tp_size)


def test_column_parallel_weight_sharding():
    """Each rank should have a different shard of the full weight."""
    in_f, out_f = 16, 32
    w_full = rand((out_f, in_f))
    layers = [ColumnParallelLinear(in_f, out_f, tp_size=2, tp_rank=r) for r in range(2)]
    for layer in layers:
        layer.load_weight(jnp.array(w_full))
    # rank 0 has rows 0:16, rank 1 has rows 16:32
    np.testing.assert_allclose(
        np.array(layers[0].weight.value), w_full[:16, :], atol=1e-6
    )
    np.testing.assert_allclose(
        np.array(layers[1].weight.value), w_full[16:, :], atol=1e-6
    )
...
def test_merged_column_shape():
    layer = MergedColumnParallelLinear(32, [64, 64])  # gate + up
    w0 = rand((64, 32), 0)
    w1 = rand((64, 32), 1)
    layer.load_weight(jnp.array(w0), shard_id=0)
    layer.load_weight(jnp.array(w1), shard_id=1)
    out = layer(jnp.ones((4, 32)))
    assert out.shape == (4, 128)  # 64+64, tp_size=1


def test_merged_column_correctness():
    """Output of merged layer must match concatenating two separate matmuls."""
    in_f = 32
    w0 = rand((64, in_f), 0)
    w1 = rand((64, in_f), 1)
    x_np = rand((4, in_f), 2)
...
def test_qkv_parallel_shape():
    ...
    # output = (num_heads + 2*num_kv_heads) * head_size = (4+4)*32 = 256
    wq = rand((num_heads * head_size, hidden))
    wk = rand((num_kv_heads * head_size, hidden))
    wv = rand((num_kv_heads * head_size, hidden))
    layer.load_weight(jnp.array(wq), shard_id="q")
    layer.load_weight(jnp.array(wk), shard_id="k")
    layer.load_weight(jnp.array(wv), shard_id="v")
...
def test_row_parallel_weight_sharding():
    """Each rank's weight must be the correct column slice."""
    in_f, out_f = 64, 32
    w_full = rand((out_f, in_f))
    for r in range(2):
